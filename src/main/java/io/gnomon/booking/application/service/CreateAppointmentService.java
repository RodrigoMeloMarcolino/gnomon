...
public class CreateAppointmentService implements CreateAppointmentUseCase {
  ...
  /**
   * @deprecated tests and adapters must use {@link BookingErrorCodes} directly.
   */
  @Deprecated
  ...
  public CreationResult create(CreateAppointmentCommand command) {
    ...
    try {
      ...
      BookingException bookingException =
          new BookingException(exception.code(), exception.getMessage());
      logFailure(bookingException, tenantId, calendarId, appointmentId);
  ...
  private static String publicErrorCode(String code) {
    return BookingErrorCodes.SLOT_UNAVAILABLE_VALIDATION.equals(code) ? "slot_unavailable" : code;
  }
}
