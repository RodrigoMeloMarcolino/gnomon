...
@RestControllerAdvice(basePackageClasses = PublicAppointmentController.class)
public class BookingExceptionHandler {
  ...
  ResponseEntity<ApiErrorResponse> handle(BookingException exception) {
    if (CreateAppointmentService.SLOT_UNAVAILABLE_VALIDATION.equals(exception.code())) {
      return error(HttpStatus.UNPROCESSABLE_ENTITY, "slot_unavailable", exception.getMessage());
    }
    HttpStatus status =
        switch (exception.code()) {
          ...
          case "phone_invalid", "validation_error" -> HttpStatus.UNPROCESSABLE_ENTITY;
  ...
  private static boolean isKnownValidationConstraint(Throwable exception) {
    return contains(exception, "ck_appointments_time_order")
